...
import type { SunPosition, SunTimes, ArcPoint } from "@/types/sun";

export function radToDeg(rad: number): number {
  return rad * (180 / Math.PI);
}

export function sunCalcAzimuthToCompass(azimuthRad: number): number {
...
export function computeArc(lat: number, lng: number, date: Date): ArcPoint[] {
